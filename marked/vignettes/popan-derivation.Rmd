...
proc.Rmark <-
  RMark::process.data(captureData %>% as.data.frame(), model = "POPAN", groups = "groupID")

ddl.Rmark <- RMark::make.design.data(proc.Rmark)

mod.Rmark <- RMark::mark(
    proc.Rmark,
    ddl.Rmark,
    model.parameters = list(
      Phi = list(formula =  ~ groupID),
      p = list(formula =  ~ groupID),
      pent = list(formula =  ~ groupID),
      N = list(formula =  ~ groupID)
    )
...
# Replace results in RMark object with marked results
mod.Rmark2 <- mod.Rmark
mod.Rmark2$results$beta$estimate <- c(mod$results$beta$Phi,
  mod$results$beta$p,
  mod$results$beta$pent,
  mod$results$beta$N)
mod.Rmark2$results$beta.vcv <- mod$results$beta.vcv

# Derive abundance
modAbundDerived <- RMark::popan.derived(proc.Rmark, mod.Rmark2)

modAbundDerived$Nbyocc
```
