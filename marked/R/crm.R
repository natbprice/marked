...
crm <-
           ...
           ...)
  {
    ...
    if (!valid.parameters(model, model.parameters))
      stop()
    parameters = setup.parameters(data.proc$model,
                                  model.parameters,
                                  data$nocc,
    ...
    for (i in 1:length(parameters))
    {
      ...
      {
        re_names = sub("^\\s+", "", sapply(strsplit(names(
          mlist$re.model
        ), "\\|"), function(x)
          x[2]))
        ...
        if ((length(re_names) > 1 ||
             re_names[1] != "time" ||  use.admb) & any(data.proc$freq > 1))
          stop(
    ...
    if (substr(model, 1, 4) == "MVMS")
    {
      ...
      else
      {
        bad_pi = sapply(split(ddl$pi$fix, ddl$pi$id), function(x) {
          ifelse(!is.null(x) &&
                   any(is.na(x)) & !(any(x[!is.na(x)] == 1)), TRUE, FALSE)
        })
        if (any(bad_pi))
          message(
      ...
      {
        bad_delta = sapply(split(
          ddl$delta$fix,
          list(ddl$delta$id, ddl$delta$occ, ddl$delta$stratum)
        ), function(x) {
          ifelse(!is.null(x) &&
                   any(is.na(x)) & !(any(x[!is.na(x)] == 1)), TRUE, FALSE)
        })
      ...
      {
        bad_Psi = sapply(split(
          ddl$Psi$fix,
          list(ddl$Psi$id, ddl$Psi$occ, ddl$Psi$stratum)
        ), function(x) {
          ifelse(!is.null(x) &&
                   any(is.na(x)) & !(any(x[!is.na(x)] == 1)), TRUE, FALSE)
        })
    ...
    if (substr(model, 1, 3) == "HMM" |
        (nchar(model) >= 4 & substr(model, 1, 4) == "MVMS"))
        ...
        (nchar(model) >= 4 & substr(model, 1, 4) == "MVMS"))
    {
      ...
      if (is.null(data.proc$strata.list) | substr(model, 1, 4) == "MVMS") {
        ...
        mx = list(
          ns = length(data.proc$strata.list$states),
      ...
      if (use.admb & model == "MVMSCJS")
      {
        ...
        xx = HMMLikelihood(
          par = unlist(initial.list$par),
          xx = data.proc$ehmat,
          mx = mx,
          type = initial.list$ptype,
          T = data.proc$nocc,
          xstart = data.proc$start,
          freq = data.proc$freq,
          fct_dmat = data.proc$fct_dmat,
          fct_gamma = data.proc$fct_gamma,
          fct_delta = data.proc$fct_delta,
          ddl = ddl,
          dml = dml,
          parameters = parameters,
          sup = sup,
      ...
      } else
      {
        ...
        runmodel = list(optim.details = as.list(summary(
          runmodel, order = "value", par.select = FALSE
        )[1,]))
        if (hessian)
          runmodel$hessian = attr(runmodel$optim.details, "details")$nhatend
        runmodel$convergence = runmodel$optim.details$convcode
        runmodel$options = list(
          accumulate = accumulate,
          initial = initial.list$par,
          method = method,
          chunk_size = chunk_size,
      ...
      if (save.matrices)
      {
        ...
        {
          dimnames(runmodel$mat$gamma)[3:4] = list(c(data.proc$strata.labels, "Dead"),
    ...
    if (!is.null(runmodel$convergence) &&
        runmodel$convergence != 0 & !use.admb)
    {
    ...
    if (!re &
        model != "MSCJS" &
        (nchar(model) < 4 | (nchar(model) >= 4 &
                             substr(model, 1, 4) != "MVMS")))
      object$results$reals = predict(object,
                                     ddl = ddl,
                                     unique = TRUE,
                                     se = hessian)
    cat(paste("\nElapsed time in minutes: ", round((
      proc.time()[3] - ptm[3]
    ) / 60, digits = 4), "\n"))
    return(object)
  }
