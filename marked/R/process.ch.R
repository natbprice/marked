...
#' character is used (1S,1W,0,2W). If comma separated, each string must contain 
...
#' FORTRAN; must be true for js because it returns additional quantities needed 
#' for entry prob.
#' @return 
...
#' \item{FtoL}{1's from first (1) to last (1) and 0's elsewhere; only if 
#' all==TRUE} 
#' \item{Fplus}{1's from occasion after first (1) to nocc(last occasion); only 
#' if all==TRUE} 
...
process.ch <- function(ch, freq = NULL, all = FALSE) {
  ...
  ymat <- matrix(
    as.numeric(unlist(strsplit(ch, ""))),
    byrow = TRUE,
    ncol = nocc,
    nrow = nch
  )
  if (suppressWarnings(all(is.numeric(as.numeric(chmat)))))
    chmat = ymat
  ymat[ymat == 0] <- NA
  ...
  if (all) {
    FtoL <- t(apply(cbind(first, last), 1, function(x, nocc) {
      return(c(rep(0, x[1]), rep(1, x[2] - x[1]), rep(0, nocc - x[2])))
    }, nocc = nocc))
    First <- t(sapply(first, function(x, nocc) {
      return(c(rep(0, x[1] - 1), rep(1, nocc - x[1] + 1)))
    }, nocc = nocc))
    Fplus <- t(sapply(first, function(x, nocc) {
      return(c(rep(0, x[1]), rep(1, nocc - x[1])))
    }, nocc = nocc))
    Lplus <- t(sapply(last, function(x, nocc) {
      return(c(rep(0, x[1]), rep(1, nocc - x[1])))
    }, nocc = nocc))
    L <- t(sapply(last, function(x, nocc) {
      return(c(rep(0, x[1] - 1), rep(1, nocc - x[1] + 1)))
    }, nocc = nocc))
